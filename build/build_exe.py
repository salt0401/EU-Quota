...
def run_pyinstaller():
    ...
    cmd = [
        ...
        # numpy >= 2.4 splits C-extension helpers into numpy._core submodules
        # that PyInstaller's hook misses; collect them all explicitly
...
def cleanup_build_artifacts():
    ...
    for folder in [PYINSTALLER_BUILD, PYINSTALLER_DIST]:
        if os.path.exists(folder):
            print(f"  Removing: {folder}")
            _force_rmtree(folder)  # plain rmtree fails on OneDrive-locked files
